...
// 승인 카탈로그 재검수. 250건/일이면 카탈로그(~3,000건)가 약 2주에 한 바퀴.
// 한때 7일 주기를 노리고 400으로 올렸다가 되돌렸다: 결제 콘솔을 확인해보니 Gemini 지출
// (월 ₩2.5만+ 페이스, 선불 크레딧 9일 만에 소진)의 대부분이 이 재검수 요청이었다.
// 방문자 20/일·수익 0인 현 단계에서 교정 주기 1주는 사치다 — 사람이 고친 분류는 이제
// user 가드로 보호되므로 주기가 늘어도 잃는 건 "AI 오분류가 좀 더 오래 남는 것"뿐이다.
const AUDIT_PER_RUN = 250;

...
// 승인 카탈로그 재검수. 카탈로그가 약 2,800건이라 400건/일이면 한 바퀴에 약 7일.
// CLIP이 라이브 카테고리에서 손을 뗀 뒤로 라이브 분류를 여기가 전담하므로, 오분류가
// 노출된 채 방치되는 기간을 줄이려고 250 -> 400으로 올렸다 (요청 수는 하루 27건 수준).
const AUDIT_PER_RUN = 400;

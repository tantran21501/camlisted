...
무료 할당량은 1일 10,000 unit인데, `search.list`(검색)는 그와 별개로 프로젝트당 하루 100회라는 별도 한도가 있습니다
(100회 × 100 unit = 10,000이라 사실상 같은 제약). `scripts/update.mjs`는 이 100회를 `SEARCH_BUDGET_PER_RUN`(기본 95,
여유분 5회)만큼만 쓰도록 예산을 관리합니다: 키워드 검색(라이브/영상 키워드, 목록이 한도보다 길어지면 날짜별로
자동 로테이션)에 쓰고 남은 만큼을 전부 채널 스캔에 투입합니다. `videos.list`/`channels.list`는 훨씬 저렴해서
(생존 확인, 국가 조회) 이 한도에 거의 영향받지 않습니다.
...
다음 실행부터 기존 목록에서도 자동으로 걸러집니다. 관리자는 사이트에서 바로 삭제할 수 있고, 삭제된 영상/채널은 차단목록에 올라 재검색에서 제외됩니다.

## 유저 제보
로그인한 유저는 상단 입력창에 유튜브 라이브 URL을 붙여넣어 새 CCTV를 등록할 수 있습니다.
등록되면 승인 대기 상태로 들어가며, 다른 유저 3명의 추천 또는 관리자 승인 시 정식 공개됩니다(7일 내 미승인 시 자동 삭제).
